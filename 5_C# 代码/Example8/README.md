...
![消费者可靠性确认](https://cdn.jsdelivr.net/gh/AlbertZhaoz/blogpic@master/2024/消费者可靠性确认.4kn95u2alna0.webp)
---

## Enabling Consumer Turn Automatic Ack To Manual
...
```
## Limit The Number Of Failed Redeliveries
This is achieved by introducing the dead letter queue and adding the message retry header x-retry-count when the message is published.
```
...
```
## Others
You need to pay attention to the idempotent of messages, there are two ways: the first way is the unique message id, the other way can be through the business code to see if it has been consumed.